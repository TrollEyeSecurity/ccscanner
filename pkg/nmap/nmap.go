...
func Scan(nmap_params *string, hosts *string, excludes *string, taskId *primitive.ObjectID, shodanKey *string) {
	...
	for _, host := range data.Host {
		...
		for _, port := range host.Ports.Port {
			var urls []string
			if port.Service.Name == "http" || port.Service.Name == "https" || port.Service.Name == "https-alt" || port.Service.Name == "ipp" || port.Service.Name == "ssl" || port.Service.Name == "unicall" || port.Service.Name == "snet-sensor-mgmt" || port.Service.Name == "zeus-admin" {
				protocol := "http://"
